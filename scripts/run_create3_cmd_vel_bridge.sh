...
start_bridge() {
  ...
  docker exec -d "$AUTONOMY_CONTAINER" bash -lc \
    "source /opt/ros/humble/setup.bash && cd '$CONTAINER_WORKDIR' && exec python3 ./scripts/create3_cmd_vel_bridge.py send --topic '$AUTONOMY_TOPIC_CMD_VEL' --host '$BRIDGE_HOST' --port '$BRIDGE_PORT'" \
    >/dev/null
  ...
  docker run -d --rm \
    ...
    "$ROBOT_IMAGE" \
    bash -lc "source /opt/ros/humble/setup.bash && exec python3 ./scripts/create3_cmd_vel_bridge.py recv --topic '$ROBOT_TOPIC_CMD_VEL' --host '$BRIDGE_HOST' --port '$BRIDGE_PORT'" \
    >/dev/null
