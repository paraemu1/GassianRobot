...
PY_SCRIPT="/robot_ws/scripts/teleop_gamecube_hidraw.py"
...
main() {
  ...
  if [[ ! -f "${PROJECT_ROOT}/scripts/teleop_gamecube_hidraw.py" ]]; then
    echo "Missing script: ${PROJECT_ROOT}/scripts/teleop_gamecube_hidraw.py" >&2
    exit 1
  fi
