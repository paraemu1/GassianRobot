...
run_utils_scan_log_path() {
  local run_dir="$1"
  echo "${run_dir}/logs/auto_scan_mission.log"
}

run_utils_is_scan_run() {
  local run_dir="$1"
  [[ -f "$(run_utils_scan_log_path "$run_dir")" ]]
}

run_utils_scan_run_mode() {
  local run_dir="$1"
  local log_file

  log_file="$(run_utils_scan_log_path "$run_dir")"
  if [[ ! -f "$log_file" ]]; then
    echo "unknown"
    return 0
  fi

  if grep -Eq '^\[[0-9:]+\] dry_run=1$' "$log_file"; then
    echo "dry-run"
  else
    echo "live"
  fi
...
run_utils_scan_run_result() {
  ...
  if grep -Eq '^\[[0-9:]+\] mission complete$' "$log_file"; then
    echo "complete"
  else
    echo "needs-review"
  fi
...
run_utils_scan_history_lines() {
  local repo_root="$1"
  local run_dir has_db has_waypoints

  while IFS= read -r run_dir; do
    [[ -z "$run_dir" ]] && continue
    if ! run_utils_is_scan_run "$run_dir"; then
      continue
    fi
    ...
    if [[ -f "${run_dir}/live_scan_waypoints.tsv" ]]; then
      has_waypoints="yes"
    else
      has_waypoints="no"
    fi
    ...
    printf '%s|%s|%s|%s|%s\n' \
      "$run_dir" \
      "$(run_utils_scan_run_mode "$run_dir")" \
      "$(run_utils_scan_run_result "$run_dir")" \
      "$has_db" \
      "$has_waypoints"
