...
Options:
  ...
  --open-browser           Wait for the viewer port, then open http://localhost:<port>.
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  if [[ "$OPEN_BROWSER" -eq 1 ]]; then
    echo "Browser: would open http://localhost:${PORT}"
  fi
  exit 0
fi
...
PY
  ...
  if [[ "${wait_code:-1}" -eq 0 ]]; then
    if command -v xdg-open >/dev/null 2>&1; then
      ...
      echo "Browser open requested: http://localhost:${PORT}"
    else
      echo "xdg-open not found; open this URL manually: http://localhost:${PORT}"
    fi
  else
    echo "Viewer port did not open within ${BROWSER_WAIT_SEC}s; open manually when ready: http://localhost:${PORT}"
  fi
fi
