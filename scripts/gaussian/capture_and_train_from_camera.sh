...
#
# Example:
# ./scripts/gaussian/capture_and_train_from_camera.sh --scene lab_quicktest --duration 20 --downscale 2
...
usage() {
  ...
  echo ""
  echo "Flags:"
  echo "  --scene <name>          create run with scripts/run_tools/init_run_dir.sh"
...
if [[ -n "$SCENE_NAME" ]]; then
  "${REPO_ROOT}/scripts/run_tools/init_run_dir.sh" "$SCENE_NAME"
...
if [[ "$SOURCE" == "oak" ]]; then
  "${REPO_ROOT}/scripts/robot/record_oak_rgb_video.sh" \
    --output "$CAPTURE_MP4" \
    --duration "$DURATION" \
    --width "$WIDTH" \
    --height "$HEIGHT" \
...
"${REPO_ROOT}/scripts/gaussian/prepare_gs_input_from_run.sh" --run "$RUN_DIR"

echo "[4/4] Processing + training + export"
if [[ "$USE_HOST" -eq 1 ]]; then
  "${REPO_ROOT}/scripts/gaussian/process_train_export.sh" \
    --run "$RUN_DIR" \
    --from-run-env \
    --downscale "$DOWNSCALE" \
    --host
