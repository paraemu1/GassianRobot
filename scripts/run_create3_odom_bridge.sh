...
start_bridge() {
  ...
  docker exec -d "$AUTONOMY_CONTAINER" bash -lc \
    "source /opt/ros/humble/setup.bash && cd '$CONTAINER_WORKDIR' && exec python3 ./scripts/create3_odom_bridge.py recv --topic '$AUTONOMY_TOPIC_ODOM' --host '$BRIDGE_HOST' --port '$BRIDGE_PORT' --poll-hz '$ODOM_POLL_HZ'" \
    >/dev/null
  ...
  docker run -d --rm \
    ...
    "$ROBOT_IMAGE" \
    bash -lc "source /opt/ros/humble/setup.bash && exec python3 ./scripts/create3_odom_bridge.py send --topic '$ROBOT_TOPIC_ODOM' --host '$BRIDGE_HOST' --port '$BRIDGE_PORT'" \
    >/dev/null
