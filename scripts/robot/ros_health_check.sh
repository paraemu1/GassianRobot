...
# Override defaults with env vars, e.g. RGB_TOPIC=/camera/image_raw ./scripts/robot/ros_health_check.sh
...
run_in_container() {
  ...
  if ros_container_is_running "$ROS_CONTAINER"; then
    exec docker exec -i \
      ...
      "$ROS_CONTAINER" \
      bash -lc "source /opt/ros/humble/setup.bash && cd '$CONTAINER_WORKDIR' && exec ./scripts/robot/ros_health_check.sh"
  fi
