...
usage() {
  cat <<EOF
Usage: ./scripts/robot/launch_live_auto_scan.sh [start|start-only|mission|stop|status]
...
ensure_runtime_image() {
  ...
  if [[ "$AUTO_BUILD_RUNTIME_IMAGE" != "1" ]]; then
    fail "Missing Docker image: $image. Build it with ./scripts/build/build_robot_runtime_image.sh or set AUTO_BUILD_RUNTIME_IMAGE=1."
  fi

  log "Missing Docker image $image. Building it now..."
  "${SCRIPT_DIR}/../build/build_robot_runtime_image.sh"
}
...
start_oak() {
  start_runtime_job "OAK camera" "${LOG_DIR}/oak_camera.log" "./scripts/robot/run_oak_camera.sh"
...
start_rtabmap() {
  ...
  start_runtime_job \
    "RTAB-Map" \
    "${LOG_DIR}/rtabmap.log" \
    "DELETE_DB_ON_START=${DELETE_DB_ON_START} VISUAL_ODOMETRY=${VISUAL_ODOMETRY_FOR_AUTOSCAN} RTABMAP_DB_PATH=${db_path_container} ./scripts/robot/run_rtabmap_rgbd.sh"
...
start_nav2() {
  start_runtime_job "Nav2" "${LOG_DIR}/nav2.log" "./scripts/robot/run_nav2_with_rtabmap.sh"
