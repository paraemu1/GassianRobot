...
BRIDGE_CONTAINER="${BRIDGE_CONTAINER:-create3_cmd_vel_bridge}"
BRIDGE_HOST="${BRIDGE_HOST:-127.0.0.1}"
BRIDGE_PORT="${BRIDGE_PORT:-18911}"
AUTONOMY_TOPIC_CMD_VEL="${AUTONOMY_TOPIC_CMD_VEL:-/cmd_vel}"
ROBOT_TOPIC_CMD_VEL="${ROBOT_TOPIC_CMD_VEL:-/cmd_vel}"
...
bridge_sender_running() {
  docker exec "$AUTONOMY_CONTAINER" bash -lc "pgrep -af 'create3_cmd_vel_bridg[e].py send' >/dev/null"
}
...
stop_bridge() {
  ...
  if docker ps --format '{{.Names}}' | grep -Fxq "$AUTONOMY_CONTAINER"; then
    docker exec "$AUTONOMY_CONTAINER" bash -lc "pkill -f 'create3_cmd_vel_bridg[e].py send' >/dev/null 2>&1 || true"
  fi
}
...
status_bridge() {
  ...
  if bridge_sender_running 2>/dev/null; then
    echo "[PASS] cmd_vel sender running in $AUTONOMY_CONTAINER"
  else
    echo "[FAIL] cmd_vel sender not running in $AUTONOMY_CONTAINER"
    ok=1
  fi

  if bridge_receiver_running; then
    echo "[PASS] robot bridge container running: $BRIDGE_CONTAINER"
  else
    echo "[FAIL] robot bridge container not running: $BRIDGE_CONTAINER"
    ok=1
  fi
...
start_bridge() {
  ...
  docker exec -d "$AUTONOMY_CONTAINER" bash -lc \
    "source /opt/ros/humble/setup.bash && cd '$CONTAINER_WORKDIR' && exec python3 ./scripts/robot/create3_cmd_vel_bridge.py send --topic '$AUTONOMY_TOPIC_CMD_VEL' --host '$BRIDGE_HOST' --port '$BRIDGE_PORT'" \
    >/dev/null
  ...
  docker run -d --rm \
    ...
    "$ROBOT_IMAGE" \
    bash -lc "source /opt/ros/humble/setup.bash && exec python3 ./scripts/robot/create3_cmd_vel_bridge.py recv --topic '$ROBOT_TOPIC_CMD_VEL' --host '$BRIDGE_HOST' --port '$BRIDGE_PORT'" \
    >/dev/null
