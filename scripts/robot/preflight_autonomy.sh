...
wait_for_create3_base_health() {
  ...
  while (( SECONDS < deadline )); do
    if "${REPO_ROOT}/scripts/robot/create3_base_health_check.sh" >"$output_file" 2>&1; then
      return 0
    fi
  ...
  "${REPO_ROOT}/scripts/robot/create3_base_health_check.sh" >"$output_file" 2>&1 || true
  return 1
}

for s in \
  "${REPO_ROOT}/scripts/robot/run_robot_runtime_container.sh" \
  "${REPO_ROOT}/scripts/robot/run_oak_camera.sh" \
  "${REPO_ROOT}/scripts/robot/run_rtabmap_rgbd.sh" \
  "${REPO_ROOT}/scripts/robot/run_nav2_with_rtabmap.sh" \
  "${REPO_ROOT}/scripts/robot/run_create3_cmd_vel_bridge.sh" \
  "${REPO_ROOT}/scripts/robot/run_create3_odom_bridge.sh" \
  "${REPO_ROOT}/scripts/robot/create3_dock_control.sh" \
  "${REPO_ROOT}/scripts/robot/create3_base_health_check.sh" \
  "${REPO_ROOT}/scripts/robot/create3_motion_ready_check.sh" \
  "${REPO_ROOT}/scripts/robot/send_nav2_goal.sh" \
  "${REPO_ROOT}/scripts/robot/record_oak_rgb_video.sh"; do
  must_file "$s"
done
...
if [[ "$NEED_ROBOT" == "1" ]]; then
  ...
  if [[ "$REQUIRE_CMD_VEL_BRIDGE" == "1" ]]; then
    if "${REPO_ROOT}/scripts/robot/run_create3_cmd_vel_bridge.sh" status >/tmp/create3_cmd_vel_bridge.out 2>&1; then
  ...
  if [[ "$REQUIRE_ODOM_BRIDGE" == "1" ]]; then
    if "${REPO_ROOT}/scripts/robot/run_create3_odom_bridge.sh" status >/tmp/create3_odom_bridge.out 2>&1; then
