...
check_create3_motion_ready() {
  ...
  set +e
  out="$(bash "${REPO_ROOT}/scripts/robot/create3_motion_ready_check.sh" 2>&1)"
  rc=$?
  set -e
...
capture_still_image() {
  ...
  if docker ps --format '{{.Names}}' | grep -Fxq "$ROS_CONTAINER"; then
    ...
    docker exec "$ROS_CONTAINER" bash -lc \
      "source /opt/ros/humble/setup.bash && cd '${CONTAINER_WORKDIR:-/robot_ws}' && python3 ./scripts/robot/capture_oak_still.py --output '$output_container' --width '$CAPTURE_WIDTH' --height '$CAPTURE_HEIGHT'"
    return
  fi
