...
wait_for_odom_input() {
  ...
  echo "Start ./scripts/robot/run_create3_odom_bridge.sh start or use VISUAL_ODOMETRY=true if you intentionally want RTAB-Map VO." >&2
  return 1
}
...
if [[ "${IN_RTABMAP_RGBD_CONTAINER:-0}" != "1" && "$PREFER_RUNNING_CONTAINER" == "1" ]] && ros_container_is_running "$ROS_CONTAINER"; then
  exec docker exec -i \
    ...
    "$ROS_CONTAINER" \
    bash -lc "source /opt/ros/humble/setup.bash && cd '$CONTAINER_WORKDIR' && exec ./scripts/robot/run_rtabmap_rgbd.sh"
fi
...
if [[ "$DEPTH_TOPIC" == "$GASSIAN_DEFAULT_DEPTH_TOPIC" && "$CAMERA_INFO_TOPIC" == "$GASSIAN_DEFAULT_CAMERA_INFO_TOPIC" ]]; then
  ...
  echo "         Prefer /oak/depth/image_raw when it exists, or validate the live timestamps/frame_ids with ./scripts/robot/check_rtabmap_sync.sh before mapping." >&2
fi
