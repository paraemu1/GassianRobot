...
PY_SCRIPT="/robot_ws/scripts/robot/teleop_drive_app.py"
...
main() {
  ...
  if [[ ! -f "${PROJECT_ROOT}/scripts/robot/teleop_drive_app.py" ]]; then
    echo "Missing script: ${PROJECT_ROOT}/scripts/robot/teleop_drive_app.py" >&2
    exit 1
  fi
