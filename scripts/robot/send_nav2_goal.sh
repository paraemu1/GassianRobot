...
# Usage:
#   ./scripts/robot/send_nav2_goal.sh 1.5 0.0 0.0 1.0
...
run_in_container() {
  ...
  if docker ps --format '{{.Names}}' | grep -Fxq "$ROS_CONTAINER"; then
    exec docker exec -i \
      ...
      "$ROS_CONTAINER" \
      bash -lc "source /opt/ros/humble/setup.bash && cd '$CONTAINER_WORKDIR' && exec ./scripts/robot/send_nav2_goal.sh '$X' '$Y' '$QZ' '$QW'"
  fi
