...
capture_still_image() {
  ...
  if docker ps --format '{{.Names}}' | grep -Fxq "$ROS_CONTAINER"; then
    ...
    docker exec "$ROS_CONTAINER" bash -lc "source /opt/ros/humble/setup.bash && cd '${CONTAINER_WORKDIR:-/robot_ws}' && python3 ./scripts/capture_oak_still.py --output '$output_container' --width '$CAPTURE_WIDTH' --height '$CAPTURE_HEIGHT'"
    return
  fi
