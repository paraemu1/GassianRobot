...
Usage:
  ./scripts/test_gs_tui.sh [--keep-run]
...
create_fixture_run() {
  ...
  RUN_ROOT="${REPO_ROOT}/runs" "${SCRIPT_DIR}/init_run_dir.sh" "$scene" >/dev/null
...
run_step "Build training images dry-run with passthrough flags" "${SCRIPT_DIR}/build_jetson_training_images.sh" --dry-run --no-cache --pull --progress plain
run_step "Validate docker builds dry-run" "${SCRIPT_DIR}/validate_docker_builds.sh" --mode clean --target all --dry-run
run_step "Training status works on explicit run" "${SCRIPT_DIR}/training_job_status.sh" --run "$run_dir_main"

run_step_not_expect "Latest training run excludes camera_health" "camera_health" "${SCRIPT_DIR}/start_gaussian_training_job.sh" --run latest --dry-run
run_step_not_expect "Latest viewer run excludes camera_health" "camera_health" "${SCRIPT_DIR}/start_gaussian_viewer.sh" --run latest --dry-run
run_step_expect "Watch logs resolves latest train log run" "Run:" "${SCRIPT_DIR}/watch_gaussian_training_job.sh" --run latest --dry-run --no-follow

run_step "Delete run soft-delete flow" "${SCRIPT_DIR}/delete_run.sh" --run "$run_dir_delete" --yes
...
if [[ -z "$trash_entry" ]]; then
  ...
  run_step "Restore run flow" "${SCRIPT_DIR}/restore_run.sh" --entry "$(basename "$trash_entry")"
fi

run_step "Purge trash dry-run" "${SCRIPT_DIR}/purge_run_trash.sh" --older-than-days 0 --dry-run
run_step "Cleanup stale training state dry-run" "${SCRIPT_DIR}/cleanup_stale_training_state.sh" --dry-run
